...
#   output txtfileout3 of trainstops with trips per hour in day for one day of the week -'trainstops_w_tph_summed_over_week'+'_'+sserviceweekstartdate+'_'+gtfsdate+'.txt'
...
#
def main(gtfsdate, gtfspath, gtfsdirbase, processedpath, serviceweekstartdate, dayofweek): # trips per hour in day for one day of the week. 
	# input:
	...
	txtfileout3 = 'trainstops_w_tph_for_oneday_of_week'+'_'+dayofweek+'_'+sserviceweekstartdate+'_'+gtfsdate+'.txt' # trainstops with trips per hour in day summed over one week 
	...
	dayofweektocount_i = keylist.index(dayofweek)
	...
	while ((count < maxfilelinecount) and (sline != '')):
		...
		# print in_id
		[service_id,sunday,monday,tuesday,wednesday,thursday,friday,saturday,start_date,end_date] = slinelist 
		slinelistoneday = [service_id,'0','0','0','0','0','0','0',start_date,end_date]
		slinelistoneday[dayofweektocount_i] = slinelist[dayofweektocount_i] 
		calendar_dict[in_id] = slinelistoneday
		sstartservicedate = min(sstartservicedate, slinelist[start_date_i])
		sendservicedate = max(sendservicedate, slinelist[end_date_i])
		#print calendar_dict[in_id]
		#print calendar_dict[
		#print '------------------'
		count += 1
		sline = filein.readline()
	...
	for stop_id, [stop_lat, stop_lon, tripsatstop_dict, tphlist, totaltpwatstop] in stops_dict.iteritems():
		...
		for trip_id, timeatstoplist in tripsatstop_dict.iteritems():
			...
			for ordcalendardate in range(max(startcalendardate.toordinal(),startservicedate.toordinal()),min(endcalendardate.toordinal(),endservicedate.toordinal())+1):
				calendardate = date.fromordinal(ordcalendardate)
				calendardayofweek = calendardate.weekday() # Monday is 0 and Sunday is 6
				...
				if tripcountforday > 0 :
					...
					if deltatimetripatstop > 100 : print 'deltatimetripatstop > 100, stop_id, trip_id, mintimetripatstop, maxtimetripatstop, deltatimetripatstop : ', stop_id, trip_id, mintimetripatstop, maxtimetripatstop, deltatimetripatstop
