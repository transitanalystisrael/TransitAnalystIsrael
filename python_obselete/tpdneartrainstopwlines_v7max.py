...
postsline = 'trainstop_id,line_name,opdneartrainstop,linestopsneartrainstop,averagetpdatstop,mediantpdatstop,maxtpdatstop\n' # added maxtpd
fileout.write(postsline)
...
#   and collect stopsforoutput_dict for js output
	...
	for line_name, [opd,tpdlist] in sorted(trainstop_tpdperline_dict.iteritems(), reverse=True, key=lambda(k,v): (v[0]/len(v[1]))):
		...
		maxtpdperline = max(tpdlist) # added maxtpd
		...
		postsline = trainstop_id+','+line_name+','+str(opd)+','+str(len(tpdlist))+','+str(averagetpdperline)+','+str(int(np.median(np.array(tpdlist))))+','+str(maxtpdperline)+'\n'
		fileout.write(postsline)
		
		#stopsforoutput_dict[trainstop_id][4][line_name] = averagetpdperline # collect tpdperline dict in stopsforoutput_dict
		stopsforoutput_dict[trainstop_id][4][line_name] = maxtpdperline # collect tpdperline dict in stopsforoutput_dict # changed average to max
		if line_name == '2-' : # train service
			...
			total_bus_tpd += maxtpdperline # changed average to max
