...
def main(gtfsdate, gtfsparentpath, gtfsdirbase, pathout, sstarttime, sstoptime, freqtpdmin):
	...
	for route_id, tpdpershapedict in routeswtpdpershape_dict.iteritems():
		for trip_shape_id, [shapetpdlist,tpw,maxtpd] in tpdpershapedict.iteritems():
			tpwperrouteshape = 0
			...
			for route_id2, [agency_id2,route_short_name2,route_long_name2,route_desc2,route_type2, tripset2, tpdlist2, totaltpdperroute2, stopset2, direction_id2] in routes_dict.iteritems():
				...
				if same_or_similar and (route_id != route_id2) and (route_id2 not in used_route_id_set) : # same stops or (similar stops and same name) for both routes and not same route_id
					...
					if totaltpdperroute >= totaltpdperroute2: 
						...
						out_route_list = [route_list2[0],agency_id2,route_short_name2,route_long_name2,route_desc2,route_type2,map(addtpd,out_route_list[6],tpdlist2),out_route_list[7]+totaltpdperroute2]
					#print out_route_list[0]
