...
def main(gtfspathin, gtfspathout, geo_filter):
    ...
    #
    # scan stops by lat-lon and create keep_stop_id_set
    keep_stop_set = GTFS_geo_filter_util.scan_stops_by_lat_lon(gtfspathin, gtfspathout, gtfs_config.MAX_STOPS_COUNT, geo_filter)
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    # print list(keep_stop_set)[:2], '... ', list(keep_stop_set)[-3:-1]
    #
    # scan stop_times by keep_stop_set to create keep_trip_id_set
    keep_trip_set = GTFS_geo_filter_util.scan_gtfsfile_by_in_set_gen_out_set(gtfspathin, 'stop_times.txt', gtfs_config.MAX_STOP_TIMES_COUNT, keep_stop_set, 'stop_id', 'trip_id')
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    ...
    keep_stop_set = GTFS_geo_filter_util.filter_gtfsfile_by_in_set_gen_out_set(gtfspathin, gtfspathout, 'stop_times.txt', gtfs_config.MAX_STOP_TIMES_COUNT, keep_trip_set, 'trip_id', 'stop_id')
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    # print list(keep_stop_set)[:8], '... ', list(keep_stop_set)[-9:-1]
    #
    # filter in stops by keep_stop_id_set
    # print filter_stops_by_stop_set(gtfspathin, gtfspathout, keep_stop_set)
    print(GTFS_geo_filter_util.filter_gtfsfile_by_in_set(gtfspathin, gtfspathout, 'stops.txt', gtfs_config.MAX_STOPS_COUNT, keep_stop_set, 'stop_id'))
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    #
    # filter in trips by keep_trip_id_set and create keep_route_id_set and keep_service_id_set and keep_shape_id_set
    keep_route_set = GTFS_geo_filter_util.filter_gtfsfile_by_in_set_gen_out_set(gtfspathin, gtfspathout, 'trips.txt', gtfs_config.MAX_TRIPS_COUNT, keep_trip_set, 'trip_id', 'route_id')
    keep_service_set = GTFS_geo_filter_util.filter_gtfsfile_by_in_set_gen_out_set(gtfspathin, gtfspathout, 'trips.txt', gtfs_config.MAX_TRIPS_COUNT, keep_trip_set, 'trip_id', 'service_id')
    keep_shape_set = GTFS_geo_filter_util.filter_gtfsfile_by_in_set_gen_out_set(gtfspathin, gtfspathout, 'trips.txt', gtfs_config.MAX_TRIPS_COUNT, keep_trip_set, 'trip_id', 'shape_id')
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    ...
    keep_agency_set = GTFS_geo_filter_util.filter_gtfsfile_by_in_set_gen_out_set(gtfspathin, gtfspathout, 'routes.txt', gtfs_config.MAX_ROUTES_COUNT, keep_route_set, 'route_id', 'agency_id')
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    # print list(keep_agency_set)[:8], '... ', list(keep_agency_set)[-9:-1]
    #
    # filter in agency by keep_agency_id_set
    print(GTFS_geo_filter_util.filter_gtfsfile_by_in_set(gtfspathin, gtfspathout, 'agency.txt', gtfs_config.MAX_AGENCY_COUNT, keep_agency_set, 'agency_id'))
    localtime = time.asctime( time.localtime(time.time()) )
    print("Local current time :", localtime)
    #
    # filter in calendar by keep_service_id_set
    print(GTFS_geo_filter_util.filter_gtfsfile_by_in_set(gtfspathin, gtfspathout, 'calendar.txt', gtfs_config.MAX_CALENDAR_COUNT, keep_service_set, 'service_id'))
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
    #
    # filter in shapes by keep_shape_id_set
    print(GTFS_geo_filter_util.filter_gtfsfile_by_in_set(gtfspathin, gtfspathout, 'shapes.txt', gtfs_config.MAX_SHAPES_COUNT, keep_shape_set, 'shape_id'))
    print("Local current time :", time.asctime( time.localtime(time.time()) ))
