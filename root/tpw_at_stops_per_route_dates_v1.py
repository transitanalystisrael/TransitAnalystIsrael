#!/usr/bin/env python
# -*- coding: utf-8 -*-
# count the number of trips per day per route at all stops
...
#   sserviceweekstartdate = '20181021' or from dates list
#   gtfsdate = '20181021' or from dates list
#   gtfsdir = 'israel'+gtfsdate
#
# outputs:
#   output text file with: stop_id, stop_lat, stop_lon, dayofservice, route_id, tpd -'stops_w_tpd_per_route'+'_'+sserviceweekstartdate+'.csv'
#
print('----------------- count the number of trips per week per route at all stops --------------------------')
print('output js file of stops with max trips per day and tpw per route (route_id)')
...
#
# create trips per day list with service day (from start to end) and count of 0 for tripsperday and empty tpdperroute_dict
...
# trips per day at stop list with service day (from start to end) and count of 0 for tripsperday and empty tpdperroute_dict, and also stop desc
...
#   also update the total count and add tpdperline to tpdperroute_dict by looking up route_id in routes_dict to find route_short_name and agency_id
#
# output js file
...
def main(gtfsdate, gtfsparentpath, gtfsdirbase, processedpath, serviceweekstartdate):
    ...
    # output:
    textfileout = 'stops_w_tpd_per_route'+'_'+sserviceweekstartdate+'.csv'
    ...
    for [dayofservice, tripsperday, tpdperroute_dict] in tripsperdaylist:
        print(dayofservice, tripsperday, tpdperroute_dict)
    ...
    print('------------------')
    #print stops_dict
    #for stop_id, stopsdictlist in stops_dict.items():
    #    print(stop_id, stopsdictlist[1:])
    print('------------------')
    ...
    for stop_id, [stop_desc, stop_lat, stop_lon, tripset, tpdlist, totaltpdatstop] in stops_dict.items():
        ...
        for trip_id in tripset:
            ...
            line_name = route_id # smaller geojson file, but need to lookup short_name, agency_id and agency name in client app for display
        ...
        for [dayofservice, trips, tpdperroute_dict] in tpdlist: # scan all service days per stop to accumulate total trips at stop from trips per day at stop
    ...
    #
    # output text file with stop_id, stop_lat, stop_lon, dayofservice, route_id, tpd
    #
    fileout = open(gtfspathout / textfileout, 'w', encoding="utf8") # save results in file
    postsline = 'stop_id,stop_lat,stop_lon,dayofservice,route_id,tpd\n'
    fileout.write(postsline)
    for stop_id, [stop_desc, stop_lat, stop_lon, tripset, tpdlist, totaltripsatstop] in stops_dict.items():
        for [dayofservice, trips, tpdperroute_dict] in tpdlist: 
            #print(dayofservice, trips, tpdperroute_dict)
            for route_id, tpd in tpdperroute_dict.items() :
                #print(stop_id, stop_lat, stop_lon, dayofservice, route_id, tpd)
                postsline = stop_id+','+stop_lat+','+stop_lon+','+str(dayofservice)+','+route_id+','+str(tpd)+'\n'
                fileout.write(postsline)
    fileout.close()
    print (gtfspathout / textfileout)
    ...
    # stopsforoutput_dict[stop_id] = [stop_lat, stop_lon, totaltripsatstop, maxtpdatstop, averagetpdatstop, maxdaytpdperroute_dict]
    #
    count = 0
    stopsforoutput_dict = {}
    for stop_id, [stop_desc, stop_lat, stop_lon, tripset, tpdlist, totaltripsatstop] in stops_dict.items():
        maxtpdatstop = 0
        maxdaytpdperroute_dict = {}
        averagetpdatstop = round(totaltripsatstop/daysofservicetocount)
        for [dayofservice, trips, tpdperroute_dict] in tpdlist: # scan all service days per stop to find day with maxtpd and save tpdperline for that day
            if trips >= maxtpdatstop :
                maxtpdatstop = trips
                maxdaytpdperroute_dict = tpdperroute_dict
        stopsforoutput_dict[stop_id] = [stop_lat, stop_lon, totaltripsatstop, maxtpdatstop, averagetpdatstop, maxdaytpdperroute_dict]
        #print count, stop_id
        count +=1
    ...
    def getJSON(s_id):
        return {
            ...
            "properties": { 
                ...
                "maxdaytpdperroute_dict": stopsforoutput_dict[s_id][5]
            }
        }
...
for proc_date in dates_list :
    main(proc_date, 'gtfs', 'israel', 'processed', proc_date)
