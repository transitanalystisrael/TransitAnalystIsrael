...
def main(gtfsdate, gtfspath, gtfsdirbase, processedpath):
    ...
    with open(pathin / txtfilein, newline='', encoding="utf8") as f:
        ...
        for row in reader:
            #print row
            routes_dict[row[0]] = [row[1], row[4][0:5]] # route_id : [agency_id, OfficeLineId]
    #print routes_dict[:4]
    ...
    while ((count < maxfilelinecount) and (sline != '')):
        ...
        [agency_id, OfficeLineId] = routes_dict[slinelist[route_id_i]] # get agency_id and OfficeLineId from routes_dict
        if agency_id == '2' : trip_type_weight = float(cfg.train_weight) # train
        elif OfficeLineId in weight_dict :
            if weight_dict[OfficeLineId][0] == 'lrt' : trip_type_weight = float(cfg.lrt_weight)
            elif weight_dict[OfficeLineId][0] == 'brt' : trip_type_weight = float(cfg.brt_weight)
            elif weight_dict[OfficeLineId][0] == 'funic' : trip_type_weight = float(cfg.funic_weight)
            elif weight_dict[OfficeLineId][0] == 'cable' : trip_type_weight = float(cfg.cable_weight)
            else : print('**************************************error')
        else : trip_type_weight = float(cfg.bus_weight)
    ...
    band_m_s = int((m_size-1)/2)
    band_l_s = int((l_size-1)/2)
    band_l_m = int((l_size-m_size)/2)
    print('band_m_s, band_l_s, band_l_m :',band_m_s, band_l_s, band_l_m)
    grid_dict = {}
    count = 0
    newcount = 0
    for stop_id, [stop_lat, stop_lon, tripset] in stops_dict.items():
    ...
    postsline = 'grid_lat,grid_lon,transitscore,transitscore_max20230101\n'
    fileout.write(postsline)
    for grid_key, [stopset, tripset, tpdlist, rawtransitscore] in grid_dict.items():
        #print grid_key, rawtransitscore
        if rawtransitscore > 1 :
            transitscore = round(100.0*math.log10(rawtransitscore)/math.log10(maxrawtransitscore))
            transitscore_max20230101 = round(100.0*math.log10(rawtransitscore)/math.log10(float(cfg.maxrawtransitscore20230101)),2)
        else:
            transitscore = 0
            transitscore_max20230101 = 0.00
        point = grid_point(grid_key[0],grid_key[1])
        postsline = str(point[1])+','+str(point[0])+','+str(int(transitscore))+','+str(transitscore_max20230101)+'\n'
        fileout.write(postsline)
    fileout.close()
